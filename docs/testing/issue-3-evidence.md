...
- Missing workflow ID, a false/missing worker result, and a worker execution error do not send the accepted reply. WF03 sends sanitized worker error context through WF02, which records `ERROR_BIA`; WF03 then sets `OPERATION=FAILED` with that error ID and sends the safe message. If WF02 itself errors, WF03 falls back to a synthetic error ID and still does not acknowledge success; that fallback cannot guarantee an `ERROR_BIA` row.
...
- Regression tests cover missing worker ID, standard envelope identity/configuration, duplicate PREPARED/FAILED operations, branch-scoped retry authorization, missing retry payload, ambiguous topic pairs, config fingerprint/version gating, maintenance-mode denial, and the n8n dispatch/Error Handler/wait/success-gate graph.
- Fresh `npm run verify` result on the local PR worktree after these review fixes: 3 workflows validated; 123 tests passed; 0 failed. `git diff --check` is clean. This does not verify actual n8n Cloud node execution.
