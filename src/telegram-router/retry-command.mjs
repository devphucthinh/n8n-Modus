...
const failure = (errorCode, errorId) => ({ ok: false, response: { status: 'ERROR', error_code: errorCode, error_id: errorId, message: 'Không thể thực hiện yêu cầu.' }, write_plan: [] });

export function planRetry({ actorUserId, errorId, tables, now = new Date().toISOString() } = {}) {
  ...
  if (!hasPermission({ actorUserId, permissionCode: 'ADMIN_RETRY', tables, now })) return failure('USER_NOT_AUTHORIZED', `err-${id || 'unknown'}-retry-denied`);
