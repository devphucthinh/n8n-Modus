...
const schemas = {
  ...
  CONFIG_GLOBAL: [
    ...
    row('CONFIG_GLOBAL', { config_key: 'INVENTORY_TOPIC_NAME_TEMPLATE', config_value: 'Kiểm kê bia - {branch_name}', value_type: 'STRING', description_vi: 'Tên topic kiểm kê khi cần tạo mới; hỗ trợ {branch_id}/{branch_name}', trang_thai: 'ACTIVE' }),
    row('CONFIG_GLOBAL', { config_key: 'DISPATCHER_NOTIFICATION_CHAT_ID', config_value: 'CHAT_ID_CONFIGURE', value_type: 'STRING', description_vi: 'Chat nhận thông báo dispatcher và lỗi vận hành', trang_thai: 'ACTIVE' }),
    row('CONFIG_GLOBAL', { config_key: 'DISPATCHER_NOTIFICATION_THREAD_ID', config_value: 'THREAD_ID_CONFIGURE', value_type: 'STRING', description_vi: 'Topic nhận thông báo dispatcher và lỗi vận hành', trang_thai: 'ACTIVE' }),
    row('CONFIG_GLOBAL', { config_key: 'DISPATCHER_HEARTBEAT_THRESHOLD', config_value: '3', value_type: 'INTEGER', description_vi: 'Số heartbeat lỗi liên tiếp trước cảnh báo nghiêm trọng', trang_thai: 'ACTIVE' }),
  ],
  ...
  CONFIG_THONG_BAO: [
    ...
    ['DISPATCH_OUTSIDE_GRACE_WINDOW', 'Lịch kiểm kê đã quá cửa sổ cho phép; cần xử lý theo quy trình retry. Mã lỗi: {error_id}'],
    ['DISPATCH_BRANCH_INACTIVE', 'Chi nhánh đang tắt nên lịch kiểm kê được bỏ qua. Mã lỗi: {error_id}'],
    ['DISPATCH_ACTIVE_SESSION_EXISTS', 'Chi nhánh đã có phiên kiểm kê đang hoạt động. Mã lỗi: {error_id}'],
    ['DISPATCHER_CRITICAL', 'Dispatcher đang lỗi liên tiếp; cần kiểm tra vận hành. Mã lỗi: {error_id}'],
    ['DISPATCHER_RECOVERY', 'Dispatcher đã hoạt động trở lại. Mã lỗi: {error_id}'],
