...
async function buildRouterWorkflow() {
  ...
  const projectOperationFailure = node({ name: 'Project OPERATION failed', type: 'n8n-nodes-base.code', typeVersion: 2, parameters: { jsCode: "const decision = $('Router Decision').first()?.json?.decision ?? {}; const row = decision.reservation?.row; if (!row) return []; const errorResult = $('Call WF02 Error Handler').first()?.json ?? {}; const errorId = String(errorResult.response?.error_id ?? decision.worker_failure_error_id ?? '').trim() || `err-${String(row.operation_id ?? 'unknown').replace(/[^A-Za-z0-9_-]/g, '_')}-worker-failed`; return [{ json: { idempotency_key: row.idempotency_key, status: 'FAILED', error_id: errorId, updated_at: new Date().toISOString() } }];" }, position: pos(3000, -120) });
  ...
  updateOperationFailure.position = pos(3220, -120);
  const projectWorkerFailureReply = node({ name: 'Project Worker Failure Reply', type: 'n8n-nodes-base.code', typeVersion: 2, parameters: { jsCode: "const source = $('Router Decision').first()?.json ?? {}; const errorResult = $('Call WF02 Error Handler').first()?.json ?? {}; return [{ json: { reply_target: source.reply_target, text: errorResult.response?.message_safe ?? source.decision?.worker_failure_text ?? '' } }];" }, position: pos(3440, -120), notes: 'Never sends the accepted text when the worker returned false or threw; uses the sanitized WF02 message.' });
  ...
  link(connections, callErrorHandler.name, projectOperationFailure.name, 1);
  ...
  link(connections, projectWorkerFailureReply.name, splitReply.name);
