...
async function buildRouterWorkflow() {
  ...
  const projectOperationFailure = node({ name: 'Project OPERATION failed', type: 'n8n-nodes-base.code', typeVersion: 2, parameters: { jsCode: "const decision = $('Router Decision').first()?.json?.decision ?? {}; const row = decision.reservation?.row; if (!row) return []; const errorId = String(decision.worker_failure_error_id ?? '').trim() || `err-${String(row.operation_id ?? 'unknown').replace(/[^A-Za-z0-9_-]/g, '_')}-worker-failed`; return [{ json: { idempotency_key: row.idempotency_key, status: 'FAILED', error_id: errorId, updated_at: new Date().toISOString() } }];" }, position: pos(2560, -120) });
  ...
  updateOperationFailure.position = pos(2780, -120);
  const projectWorkerFailureReply = node({ name: 'Project Worker Failure Reply', type: 'n8n-nodes-base.code', typeVersion: 2, parameters: { jsCode: "const source = $('Router Decision').first()?.json ?? {}; return [{ json: { reply_target: source.reply_target, text: source.decision?.worker_failure_text ?? '' } }];" }, position: pos(3000, -120), notes: 'Never sends the accepted text when the worker returned false or threw.' });
  ...
  link(connections, executeWorker.name, projectOperationFailure.name, 1);
  link(connections, workerSuccessCheck.name, projectOperationCommit.name, 0);
  link(connections, workerSuccessCheck.name, projectOperationFailure.name, 1);
