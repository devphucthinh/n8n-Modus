...
import { gatewayCode, assembleCode } from '../workflow-src/WF01_V2_CONFIG_GATEWAY.mjs';
...
async function buildGatewayWorkflow() {
  ...
  prepareOperation.position = pos(1330, -100);
  ...
  prepareSnapshot.position = pos(1570, -100);
  const commitSnapshot = googleSheetNode('Commit CONFIG_SNAPSHOT', 'CONFIG_SNAPSHOT', 'update', { columns: { mappingMode: 'defineBelow', value: { status: 'COMMITTED' } } });
  commitSnapshot.position = pos(1810, -100);
  const commitOperation = googleSheetNode('Commit OPERATION', 'OPERATION', 'update', { columns: { mappingMode: 'defineBelow', value: { status: 'COMMITTED' } } });
  commitOperation.position = pos(2050, -100);
  const returnStatus = node({ name: 'Return Gateway Result', type: 'n8n-nodes-base.code', typeVersion: 2, parameters: { jsCode: 'return [{ json: $input.first()?.json ?? {} }];' }, position: pos(2290, -100) });
  const errorCall = node({ name: 'Call Error Handler', type: 'n8n-nodes-base.executeWorkflow', typeVersion: 1.2, parameters: { workflowId: { __rl: true, value: 'PASTE_WF02_WORKFLOW_ID', mode: 'id' }, options: { waitForSubWorkflow: true } }, position: pos(1330, 180), notes: 'V2 technical placeholder; select WF02_V2_ERROR_HANDLER after import.' });
  const nodes = [trigger, ...reads, assemble, decision, branch, prepareOperation, prepareSnapshot, commitSnapshot, commitOperation, returnStatus, errorCall];
  const connections = {};
  ...
  link(connections, branch.name, prepareOperation.name, 0);
  link(connections, branch.name, errorCall.name, 1);
  link(connections, prepareOperation.name, prepareSnapshot.name);
  link(connections, prepareSnapshot.name, commitSnapshot.name);
  link(connections, commitSnapshot.name, commitOperation.name);
