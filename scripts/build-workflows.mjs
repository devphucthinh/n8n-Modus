...
import { dispatcherCode, verifyClaimCode, workerFailureCode, dispatcherNoticeCode } from '../workflow-src/WF04_V2_DISPATCHER.mjs';
...
async function buildDispatcherWorkflow() {
  ...
  const nodes = [schedule, execute, request, callGateway, readHistory, readSessions, readHeartbeat, decide, heartbeatCheck, appendHeartbeat, noticeCheck, prepareNotice, callError, dispatchCheck, recordOutcome, dispatchNoticeCheck, prepareDispatchNotice, callDispatchNotice, claim, readClaim, verifyClaim, worker, workerErrorCheck, prepareWorkerError, callWorkerError, finalize, finalizeWrite, result];
  const connections = {};
  ...
  link(connections, verifyClaim.name, worker.name);
...
async function buildInventorySessionWorkflow() {
  ...
  const topicCheck = node({ name: 'Topic creation required?', type: 'n8n-nodes-base.if', typeVersion: 2.2, parameters: booleanIfParameters("={{$json.status === 'TOPIC_CREATE_REQUIRED'}}"), position: pos(1300, 0) });
  const prepareTopicRequest = node({ name: 'Prepare Telegram Topic Request', type: 'n8n-nodes-base.code', typeVersion: 2, parameters: { jsCode: "const request = $json.topic_request; if (!request?.chat_id || !request?.name) return []; return [{json: {chat_id: request.chat_id, name: request.name}}];" }, position: pos(1560, 180) });
  const createTopic = node({ name: 'Create Telegram Forum Topic', type: 'n8n-nodes-base.httpRequest', typeVersion: 4.2, parameters: { method: 'POST', url: '={{$credentials.baseUrl}}/bot{{$credentials.accessToken}}/createForumTopic', authentication: 'none', sendBody: true, specifyBody: 'json', jsonBody: '={{JSON.stringify({chat_id: $json.chat_id, name: $json.name})}}', options: {} }, credentials: { telegramApi: { name: TELEGRAM_CREDENTIAL } }, position: pos(1820, 180), notes: 'Uses the Telegram credential for the Bot API createForumTopic method; the token is never stored in workflow JSON.' });
  createTopic.continueOnFail = true;
  const openAfterTopicCreate = node({ name: 'Open Session With Created Topic', type: 'n8n-nodes-base.code', typeVersion: 2, parameters: { jsCode: await inventorySessionCode() }, position: pos(2080, 180) });
  ...
  const operationWrite = googleSheetNode('Prepare OPERATION', 'OPERATION', 'appendOrUpdate', { columns: { mappingMode: 'autoMapInputData', matchingColumns: ['operation_id'] } });
  operationWrite.position = pos(1820, -240);
  const topicRequired = node({ name: 'Topic mapping write required?', type: 'n8n-nodes-base.if', typeVersion: 2.2, parameters: booleanIfParameters("={{!!($('Select Session Result').first()?.json?.write_plan ?? []).some((entry) => entry.sheet === 'CONFIG_TOPIC' && entry.phase === 'PREPARE')}}"), position: pos(2080, -240) });
  const prepareTopicRow = node({ name: 'Prepare CONFIG_TOPIC row', type: 'n8n-nodes-base.code', typeVersion: 2, parameters: { jsCode: "const result = $('Select Session Result').first()?.json ?? {}; const entry = (result.write_plan || []).find((item) => item.sheet === 'CONFIG_TOPIC' && item.phase === 'PREPARE'); return entry?.row ? [{json: entry.row}] : [];" }, position: pos(2340, -480) });
  const topicWrite = googleSheetNode('Prepare CONFIG_TOPIC', 'CONFIG_TOPIC', 'appendOrUpdate', { columns: { mappingMode: 'autoMapInputData', matchingColumns: ['topic_id'] } });
  topicWrite.position = pos(2600, -480);
  ...
  const sessionWrite = googleSheetNode('Prepare PHIEN_KIEM_KE', 'PHIEN_KIEM_KE', 'appendOrUpdate', { columns: { mappingMode: 'autoMapInputData', matchingColumns: ['session_id'] } });
  ...
  const auditWrite = googleSheetNode('Prepare EVENT_LOG', 'EVENT_LOG', 'appendOrUpdate', { columns: { mappingMode: 'autoMapInputData', matchingColumns: ['event_id'] } });
  auditWrite.position = pos(2600, -100);
  const commitTopicRequired = node({ name: 'Topic mapping commit required?', type: 'n8n-nodes-base.if', typeVersion: 2.2, parameters: booleanIfParameters("={{!!($('Select Session Result').first()?.json?.commit_plan ?? []).some((entry) => entry.sheet === 'CONFIG_TOPIC' && entry.phase === 'COMMIT')}}"), position: pos(2860, -100) });
  const commitTopicRow = node({ name: 'Commit CONFIG_TOPIC row', type: 'n8n-nodes-base.code', typeVersion: 2, parameters: { jsCode: "const result = $('Select Session Result').first()?.json ?? {}; const entry = (result.commit_plan || []).find((item) => item.sheet === 'CONFIG_TOPIC' && item.phase === 'COMMIT'); return entry ? [{json: {...entry.match, ...entry.patch}}] : [];" }, position: pos(3120, -460) });
  const commitTopic = googleSheetUpdateNode('Commit CONFIG_TOPIC', 'CONFIG_TOPIC', 'topic_id', ['topic_id', 'message_thread_id', 'trang_thai']);
  commitTopic.position = pos(3380, -460);
  ...
  const commitSessionRow = node({ name: 'Commit PHIEN_KIEM_KE row', type: 'n8n-nodes-base.code', typeVersion: 2, parameters: { jsCode: "const result = $('Select Session Result').first()?.json ?? {}; const entry = (result.commit_plan || []).find((item) => item.sheet === 'PHIEN_KIEM_KE' && item.phase === 'COMMIT'); return entry ? [{json: {...entry.match, ...entry.patch}}] : [];" }, position: pos(3900, -240) });
  const commitSession = googleSheetUpdateNode('Commit PHIEN_KIEM_KE', 'PHIEN_KIEM_KE', 'session_id', ['session_id', 'status', 'updated_at']);
  ...
  const result = node({ name: 'Return Inventory Session Result', type: 'n8n-nodes-base.code', typeVersion: 2, parameters: { jsCode: "return $('Select Session Result').all();" }, position: pos(4680, -20) });
  const nodes = [execute, request, callGateway, readSessions, decide, topicCheck, prepareTopicRequest, createTopic, openAfterTopicCreate, selectSessionResult, writeRequired, prepareOperation, operationWrite, topicRequired, prepareTopicRow, topicWrite, sessionRequired, prepareSession, sessionWrite, prepareAudit, auditWrite, commitTopicRequired, commitTopicRow, commitTopic, commitSessionRequired, commitSessionRow, commitSession, commitAuditRow, commitAudit, commitOperationRow, commitOperation, result];
  const connections = {};
  link(connections, execute.name, request.name);
  link(connections, request.name, callGateway.name);
  link(connections, callGateway.name, readSessions.name);
  link(connections, readSessions.name, decide.name);
  link(connections, decide.name, topicCheck.name);
  link(connections, topicCheck.name, prepareTopicRequest.name, 0);
  link(connections, topicCheck.name, selectSessionResult.name, 1);
  link(connections, prepareTopicRequest.name, createTopic.name);
  link(connections, createTopic.name, openAfterTopicCreate.name);
  link(connections, openAfterTopicCreate.name, selectSessionResult.name);
  ...
  link(connections, operationWrite.name, topicRequired.name);
  link(connections, topicRequired.name, prepareTopicRow.name, 0);
  link(connections, topicRequired.name, sessionRequired.name, 1);
  link(connections, prepareTopicRow.name, topicWrite.name);
  link(connections, topicWrite.name, sessionRequired.name);
  ...
  link(connections, auditWrite.name, commitTopicRequired.name);
  link(connections, commitTopicRequired.name, commitTopicRow.name, 0);
  link(connections, commitTopicRequired.name, commitSessionRequired.name, 1);
  link(connections, commitTopicRow.name, commitTopic.name);
  link(connections, commitTopic.name, commitSessionRequired.name);
  ...
  link(connections, commitOperationRow.name, commitOperation.name);
  link(connections, commitOperation.name, result.name);
